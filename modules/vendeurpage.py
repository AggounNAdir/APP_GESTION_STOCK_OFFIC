...
class DefinirMotDePasseVendeurDialog(tk.Toplevel):
    def __init__(self, parent, vendeur_id, code_vendeur, nom_vendeur):
        ...
        lbl(f, "🔑 Nouveau mot de passe vendeur", 12, True).pack(anchor="w")
        lbl(f, f"Vendeur : {code_vendeur} — {nom_vendeur}", 9, color=CLR_MUTED).pack(anchor="w", pady=(0, 12))

        lbl(f, "Mot de passe (min. 6 caractères)").pack(anchor="w")
        self.pwd_var = tk.StringVar()
        entry(f, width=30, textvariable=self.pwd_var, show="•").pack(anchor="w", pady=(2, 10))

        lbl(f, "Confirmation").pack(anchor="w")
...
class VendeurPage(tk.Frame):
    def __init__(self, parent):
        ...
        # Titre
        lbl(self, "👔 Gestion des Vendeurs", 16, True).pack(pady=10)

        # Formulaire
        form_frame = tk.LabelFrame(self, text="Nouveau / Modification", bg=CLR_CARD, fg=CLR_ACCENT, padx=10, pady=10)
        form_frame.pack(fill="x", pady=5, padx=10)

        lbl(form_frame, "Code:", color=CLR_TEXT).grid(row=0, column=0, padx=5, pady=5)
        self.ent_code = entry(form_frame, width=15)
        self.ent_code.grid(row=0, column=1, padx=5, pady=5)

        lbl(form_frame, "Nom:", color=CLR_TEXT).grid(row=0, column=2, padx=5, pady=5)
        self.ent_nom = entry(form_frame, width=25)
        self.ent_nom.grid(row=0, column=3, padx=5, pady=5)

        lbl(form_frame, "Téléphone:", color=CLR_TEXT).grid(row=0, column=4, padx=5, pady=5)
        self.ent_tel = entry(form_frame, width=15)
        self.ent_tel.grid(row=0, column=5, padx=5, pady=5)

        tk.Button(form_frame, text="💾 Enregistrer", command=self.save_vendeur, bg=CLR_GREEN, fg="white", relief="flat", padx=10).grid(row=0, column=6, padx=10)
        tk.Button(form_frame, text="🆕 Nouveau", command=self.clear_form, bg=CLR_MUTED, fg="white", relief="flat", padx=10).grid(row=0, column=7, padx=5)

        # Actions sur le vendeur sélectionné
        actions_frame = tk.Frame(self, bg=CLR_BG)
        ...
        tk.Button(actions_frame, text="🔑 Définir mot de passe portail", command=self.definir_mot_de_passe,
                  bg=CLR_ACCENT, fg="white", relief="flat", padx=10, pady=5, cursor="hand2").pack(side="left", padx=4)
        ...
        # Liste
        tf, self.tree = make_tree(self, columns=("id", "code", "nom", "tel", "actif", "portail"))
        tf.pack(fill="both", expand=True, pady=10, padx=10)
        for c, label, w in [
